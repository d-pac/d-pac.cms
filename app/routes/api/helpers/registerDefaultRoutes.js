...
module.exports = function registerDefaultRoutes( baseUrl,
  ...
  const parsedUrl = path.parse( baseUrl );
  ...
  listRouter.get( apiMw.setType( parsedUrl.name, 'multiple' ), controller.list )
    .post( apiMw.setType( parsedUrl.name, 'single' ), controller.create );

  resourceRouter.all( apiMw.setType( parsedUrl.name, 'single' ) )
